...
const ApiKeyPanel: React.FC<Props> = ({ 
  ...
  const styles = {
    input: "w-full h-[36px] text-[13px] px-3 border-2 border-gray-300 rounded-lg bg-white text-gray-900 font-['Share_Tech'] focus:outline-none focus:border-blue-500 focus:shadow-[0_0_8px_rgba(37,99,235,0.4)] transition-all disabled:bg-gray-50 disabled:text-gray-400",
    label: "text-[13px] font-bold text-blue-600 font-['Share_Tech'] uppercase tracking-wider mb-1 block",
    btnMain: "flex items-center justify-center border-none font-['Share_Tech'] font-bold uppercase tracking-wider rounded-lg transition-all active:translate-y-[2px] disabled:opacity-30 disabled:translate-y-0",
  ...
  return (
    <div className="bg-white p-6 rounded-xl border-2 border-gray-300 shadow-[0_10px_30px_rgba(0,0,0,0.15)] flex flex-col font-['Share_Tech']">
      <div className="flex items-center gap-2 mb-4 border-b-2 border-blue-600 pb-2 w-fit">
        ...
        <h2 className="text-xl font-bold text-blue-600 uppercase tracking-widest leading-none">API Settings</h2>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
           ...
           <div className="flex flex-col">
              ...
              <input type="text" className={`${styles.input} bg-gray-50 border-gray-200 text-gray-400`} value={getBaseUrl()} disabled />
           </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
           <div className="flex flex-col">
              <div className="flex justify-between items-center mb-1">
                  ...
                  <button onClick={() => setIsManualModel(!isManualModel)} className="text-[11px] text-blue-500 underline font-bold">
           ...
           <div className="flex flex-col">
              ...
              <div className="flex gap-2">
                  ...
                  <button onClick={() => window.open(getConnectLink(), '_blank')} disabled={isProcessing || provider === 'GOOGLE'} className={`${styles.btnMain} ${styles.shadow3d} w-[38px] h-[38px] border-2 border-blue-600 bg-blue-50 text-blue-600`}>
        ...
        {/* Dynamic Area: Login or Keys */}
        <div className="h-[64px] flex flex-col justify-end">
          {provider === 'GOOGLE' ? (
              <button onClick={handleGoogleLogin} disabled={isProcessing} className={`${styles.btnMain} ${styles.shadow3d} w-full h-[44px] bg-white border-2 border-gray-300 text-gray-700 gap-3`}>
              ...
              <button onClick={handlePuterLogin} disabled={isProcessing} className={`${styles.btnMain} ${styles.shadow3d} w-full h-[44px] border-2 ${isPuterAuthenticated ? 'bg-green-50 border-green-500 text-green-700' : 'bg-blue-50 border-blue-500 text-blue-700'}`}>
            ...
            <div className="flex gap-2">
                <textarea placeholder="Keys (one per line)..." className={`${styles.input} h-[44px] py-2 resize-none font-mono text-[10px]`} value={bulkInput} onChange={(e) => setBulkInput(e.target.value)} />
                <input type="file" ref={fileInputRef} accept=".txt" className="hidden" onChange={handleLoadTxt} />
                <button onClick={() => fileInputRef.current?.click()} disabled={isProcessing} className={`${styles.btnMain} ${styles.shadow3d} w-[44px] h-[44px] bg-blue-50 border-2 border-blue-600 text-blue-600 flex-col gap-0.5`}>
                    <FileText size={16} />
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className={`${styles.btnMain} h-[38px] border-2 border-blue-200 bg-blue-50 text-blue-800 text-[14px]`}>
             SLOTS: {apiKeys.length}
          </div>
          <button onClick={handleAddKeys} disabled={isProcessing || provider === 'GOOGLE' || (provider === 'PUTER' && !isPuterAuthenticated) || (provider !== 'PUTER' && provider !== 'GOOGLE' && !bulkInput.trim())} className={`${styles.btnMain} ${styles.shadow3d} h-[38px] bg-blue-600 text-white border-2 border-blue-800`}>
            <Plus size={18} className="mr-1" /> {addActionLabel}
          </button>
          <button onClick={handleClearAll} disabled={isProcessing || apiKeys.length === 0} className={`${styles.btnMain} ${styles.shadow3d} h-[38px] bg-red-500 text-white border-2 border-red-700`}>
        ...
        <div className="border-2 border-gray-200 rounded-xl bg-gray-50 overflow-hidden flex flex-col h-[280px] shadow-inner">
          <div className="bg-gray-200 px-3 py-2 border-b-2 border-gray-300 flex items-center justify-between">
              <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-bold text-gray-600 uppercase tracking-widest">Worker Capacity Slots</span>
              </div>
              <div className="relative">
                  <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-24 pl-6 pr-2 py-0.5 text-[10px] border-2 border-gray-300 rounded-full bg-white focus:outline-none" />
              </div>
          </div>
          <div className="overflow-y-auto p-2 flex-1 scrollbar-thin scrollbar-thumb-gray-300">
              {filteredKeys.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-2 opacity-60">
                      <ListOrdered size={24} />
                      <span className="text-[11px] font-bold uppercase tracking-widest">Empty Slots</span>
                  </div>
              ) : (
